...
Once the `conda-forge` channel has been enabled, `libllvm11, lit, llvm, llvm-tools, llvmdev` can be installed with `conda`:
...
```
mamba install libllvm11 lit llvm llvm-tools llvmdev
```

It is possible to list all of the versions of `libllvm11` available on your platform with `conda`:
...
```
mamba search libllvm11 --channel conda-forge
```
...
mamba repoquery search libllvm11 --channel conda-forge

# List packages depending on `libllvm11`:
mamba repoquery whoneeds libllvm11 --channel conda-forge

# List dependencies of `libllvm11`:
mamba repoquery depends libllvm11 --channel conda-forge
```
