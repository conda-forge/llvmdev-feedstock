...
Once the `conda-forge` channel has been enabled, `libllvm-c20, libllvm20, lit, llvm, llvm-tools, llvm-tools-20, llvmdev` can be installed with `conda`:
...
```
conda search libllvm-c20 --channel conda-forge
```

or with `mamba`:

```
mamba search libllvm-c20 --channel conda-forge
```
...
mamba repoquery search libllvm-c20 --channel conda-forge

# List packages depending on `libllvm-c20`:
mamba repoquery whoneeds libllvm-c20 --channel conda-forge

# List dependencies of `libllvm-c20`:
mamba repoquery depends libllvm-c20 --channel conda-forge
```
