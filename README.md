...
Once the `conda-forge` channel has been enabled, `libllvm7.1, lit, llvm, llvm-tools, llvm-utils, llvmdev` can be installed with `conda`:

```
conda install libllvm7.1 lit llvm llvm-tools llvm-utils llvmdev
```

or with `mamba`:

```
mamba install libllvm7.1 lit llvm llvm-tools llvm-utils llvmdev
```
