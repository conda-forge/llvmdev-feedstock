...
Once the `conda-forge` channel has been enabled, `llvmdev` can be installed with `conda`:
...
It is possible to list all of the versions of `llvmdev` available on your platform with `conda`:
...
```
mamba search llvmdev --channel conda-forge
```
...
mamba repoquery search llvmdev --channel conda-forge

# List packages depending on `llvmdev`:
mamba repoquery whoneeds llvmdev --channel conda-forge

# List dependencies of `llvmdev`:
mamba repoquery depends llvmdev --channel conda-forge
```
