...
Once the `conda-forge` channel has been enabled, `libllvm12, lit, llvm, llvm-tools, llvmdev` can be installed with `conda`:

```
conda install libllvm12 lit llvm llvm-tools llvmdev
```

or with `mamba`:

```
mamba install libllvm12 lit llvm llvm-tools llvmdev
```

It is possible to list all of the versions of `libllvm12` available on your platform with `conda`:

```
conda search libllvm12 --channel conda-forge
```

or with `mamba`:

```
mamba search libllvm12 --channel conda-forge
```
...
mamba repoquery search libllvm12 --channel conda-forge

# List packages depending on `libllvm12`:
mamba repoquery whoneeds libllvm12 --channel conda-forge

# List dependencies of `libllvm12`:
mamba repoquery depends libllvm12 --channel conda-forge
```
