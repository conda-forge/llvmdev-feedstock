...
MAJOR_VER="${VER_ARR[0]}"
MAJOR_EXT="${MAJOR_VER}"
# v7 specific: SOVER is major.minor (libLLVM-7.1.so), unlike LLVM 8+ which uses major only
...
if [[ "${PKG_NAME}" == libllvm-c* ]]; then
    ...
    # v7 specific: libLTO uses full 3-part version (e.g. libLTO.so.7.1.0), not major.minor
    mv ./temp_prefix/lib/lib*.so.${PKG_VERSION} $PREFIX/lib || true
